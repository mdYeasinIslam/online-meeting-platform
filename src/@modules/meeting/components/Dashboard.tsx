...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    void api<MeetingList>(`/meetings?page=${page}`, {
        ...
        <>
          {!data.meetings.length && (
            <p className="my-4 text-slate-400">
              No meetings on this page. Create a meeting to get an invitation
              link.
            </p>
          )}
          <ul className="my-4 space-y-3">
            {data.meetings.map((meeting) => (
              <li
                key={meeting.roomId}
                className="flex flex-wrap justify-between gap-4 rounded-xl border border-slate-700 p-4"
              >
                <div>
                  <Link
                  ...
                  <p className="mt-1 text-sm text-slate-400">
                    {meeting.status} · Up to {meeting.maxParticipants}{" "}
                    participants
                  </p>
                </div>
